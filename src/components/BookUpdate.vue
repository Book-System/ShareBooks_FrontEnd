    ...
    <div class="container">
        ...
        <div class="sector-container">
            <div class="st-background">
                <img src="../assets/shareBooksBanner/share_books_banner_05.jpg" alt="image">
            </div>
            <div class="st-content-center" style="color: #fff;">
                <span style="font-size: 55px;">책 수정하기</span><br>
                <div style="margin-top: 15px; margin-bottom: 30px;">
                    <span style="font-size: 19px;">등록하신 책의 정보를 수정해볼까요?</span>
                </div>
            </div>
        </div>
        <!-- 메인 배너 -->
        <div class="sector2-container">
            <el-card class="box-card" shadow="hover" style="width: 700px; padding: 30px; color: #3D3D3D">
                <div style="text-align: left;">
                    <span style="font-size: 18px;">변경사항이 있으신가요? 책 정보를 수정해보세요!</span><br>
                    <div style="display: flex; justify-content: left; align-items: center;">
                        <img src="../assets/shareBooksIcon/share_books_icon_13.png" alt="image" style="width: 40px; height: 40px; margin-right: 10px;">
                        <span style="font-size: 40px;">책 수정하기</span>
                    </div>
                </div>
                <!-- 글제목 -->
                ...
                <div style="text-align: left; font-size: 18px; margin-top: 18px;">
                    <span>대여료</span><br>
                    <el-input placeholder="대여료를 입력하세요" v-model="bookData.price" @blur="handleCheckPrice" style="margin-top: 7px;"></el-input>
                    <div>
                ...
                <div style="text-align: left; font-size: 18px; margin-top: 18px;">
                    ...
                    :on-remove="handleRemove" :on-success="handlePictureCardPreview" :on-exceed="handleExceed" :file-list="bookData.fileList">
                ...
                <!-- 글 수정하기 버튼 -->
                <el-button type="info" style="background-color: #5B6EBC; font-size: 18px; width: 100%; margin-top: 20px; font-family: 'Jua', sans-serif;" @click="handleBookUpdate">글 수정하기</el-button><br>
            </el-card>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            // 책 정보 가져오기
            async handleGetBookData() {
                let url = `/REST/api/book/detail?bookno=${this.bookno}`;
                let headers = {"Content-Type" : "application/json"};
                let response = await axios.get(url, {headers});
                if(response.data.result === 1) {
                    // 기존 책 정보 넣기
                    this.bookData.title = response.data.data.title;
                    this.bookData.content = response.data.data.content;
                    this.bookData.zipcode = response.data.data.zipcode;
                    this.bookData.address = response.data.data.address;
                    this.bookData.price = response.data.data.price;
                    this.bookData.category = response.data.data.category_Name;
                    this.bookData.tag = response.data.data.tag;
                    this.bookData.bookTitle = response.data.data.book_Title;
                    this.bookData.bookContent = response.data.data.book_Content;
                }
                // 책 이미지 개수 가져오기
                url = `/REST/api/book/image/count?bookno=${this.bookno}`;
                ...
                if(response.data.result > 0) {
                    // 이미지 개수
                    imageCount = response.data.result;
                    // 이미지 개수만큼 반복수행
                    for(let i = 0; i < imageCount; i++) {
                        const temp = {
                            priority: i,
                            url: `http://localhost:9090/REST/api/book/image?bookno=${this.bookno}&priority=${i}`
                        }
                        this.bookData.fileList.push(temp);
                    }
                }
            },
            // 카카오 맵
            ...
            async handleBookUpdate() {
                if(this.bookData.title !== '' && this.bookData.content !== '' && this.bookData.address !== '' && this.bookData.price !== '' && this.bookData.category !== '' && this.bookData.tag !== '' && this.bookData.bookTitle !== '' && this.bookData.bookContent !== ''){
                    const url = '/REST/api/book/update';
                    const headers = {"Content-Type" : "multipart/form-data", "token" : sessionStorage.getItem("TOKEN")};
                    const formData = new FormData();
                    formData.append("bookNo", this.bookno);
                    ...
                    formData.append("category", parseInt(this.bookData.category));
                    ...
                    if(response.data.result === 1) {
                        alert("글 수정을 완료했습니다.");
                        return this.$router.push({path: '/book/detail', query: {bookno: this.bookno}});
                    }
                }else{
